...
class __BaseForm(forms.ModelForm):
	...
	class Meta:
		...
		fields = ['title', 'location', 'description', 'private',
			    'event', 'device', 'evidence', 'person', 'company',
			    'category', 'status', 'priority', 'authorisation',
			    'manager', 'assigned_by', 'assigned_to']
		...
		self.helper.layout = Layout(
            ...
            Fieldset(
				...
				Field(
                    'event', 
					id="event-field", 
					css_class="eventfield",
					autocomplete='on',
					title=self.fields['event'].help_text,
					placeholder=_("Event"),
					wrapper_class='col-md-9'),
...
class CaseTaskCreateForm(BaseForm):
...
class CaseTaskUpdateForm(BaseForm):
